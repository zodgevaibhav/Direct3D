...
ID3D11Buffer *gpID3D11Buffer_hal_ConstantBuffer = NULL; //constant buffer 
ID3D11Buffer *gpID3D11Buffer_domain_ConstantBuffer = NULL; //constant buffer 
ID3D11Buffer *gpID3D11Buffer_pixel_ConstantBuffer = NULL; //constant buffer 


ID3D11InputLayout *gpID3D11InputLayout = NULL; //

float gNumberOfSegmentsUniform;
float gNumberOfStripsUniform=1.0f;
unsigned int gLineColorUniform;
unsigned int gNumberOfLineSegment=1;

struct CBUFFER_HAL_SHADER //Size of CBUFFER should be more than 16 byte and should be in multiple of 16
{
	XMVECTOR Hal_Constant_Function_Param; //0th number of strips, 1st number of segments
};
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[] = TEXT("D3D11:Bezier Curve Tessellation Shader");
	bool bDone = false;
	...
	hwnd = CreateWindow(szClassName,
		TEXT("D3D11:Bezier Curve Tessellation Shader"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	*/
	...
	const char *vertexShaderSourceCode =   //Vertex shader output if going to hal shader then it must be structured output
	...
	*/
	...
	const char *hullShaderSourceCode =
		...
		"{"\
		"float4 hal_constant_function_param;"\
		"};"\
		"struct vertex_output" \
		"{" \
		"float4 position:POSITION;" \
		"};" \
		"struct hal_constant_output"\
		"{"\
		"float edges[2]:SV_TESSFACTOR;"\
		"};"\

		"hal_constant_output halConstantFunction(void)"\
		"{"\
		"hal_constant_output halOutPut;"\
		"float numberOfStrips=float(hal_constant_function_param[0]);"\
		"float numberOfSegments=float(hal_constant_function_param[1]);"\
		"halOutPut.edges[0]=numberOfStrips;"\
		"halOutPut.edges[1]=numberOfSegments;"\
		"return halOutPut;"\
		"}"\
		...
		"[patchconstantfunc(\"halConstantFunction\")]"\

		"hal_output main(InputPatch<vertex_output,4> input_patch, uint i:SV_OUTPUTCONTROLPOINTID)"\
		"{"\
		"hal_output output;"\
	...
	//Step 3: Compile the shader
	/*Shader compiler exe name is fxe.exe called as Effect-Compiler tool */
	hr = D3DCompile(hullShaderSourceCode, // Vertex shader source code
		lstrlenA(hullShaderSourceCode) + 1, //length of shader source code. 'A' character indicate we want to use ANCI function. +1 is created to accomodate null terminated character at the end of string
		"HS", //name of Macro, which indicates that while compiling create VertexShader
		NULL, // struct of D3D_SHADER_MACRO, if we want to use external shader
		D3D_COMPILE_STANDARD_FILE_INCLUDE, //Include standerd file while compiling the shader program
		"main", //name of entry point function. It is not necessory to use name as 'main'
		"hs_5_0", //shader feature level. Curently level 5 is latest
		0, //Compiler constant. Ex. debug, optimization, validation
		0, // Effect constant. Ex. effect debug, effect optimize, effect validation
		&pID3DBlob_HullShaderCode, //Compilerd source code in out parameter
		&pID3DBlob_Error  //if any error, will be returned in output parameter
		);
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "Hal shader compilation failed: %s.\n", (char *)pID3DBlob_Error->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Hal shader compilation successful.\n");
		fclose(gpFile);
	}
	...
	*/
	hr = gpID3D11Device->CreateHullShader(pID3DBlob_HullShaderCode->GetBufferPointer(),
		...
		NULL,
		&gpID3D11HullShader); //Out parameter where verted shader object will be stored
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Hull shader object creation failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "HUll shader object creation success.\n");
		fclose(gpFile);
	}
	...
	const char *domainShaderSourceCode =
		"cbuffer constantBuffer"\
		"{"\
		"float4x4 WorldViewPerspeciveMatrix;"\
		"};"\
		...
		"domain_output main(hal_constant_output input, OutputPatch<hal_output,4> output_patch,float2 tessCord:SV_DomainLocation)"\
		"{"\
		...
		"output.position=mul(WorldViewPerspeciveMatrix , p4);"\
		"return(output);"\
		"}";
	...
	//Step 3: Compile the shader
	/*Shader compiler exe name is fxe.exe called as Effect-Compiler tool */
	hr = D3DCompile(domainShaderSourceCode, // Vertex shader source code
		lstrlenA(domainShaderSourceCode) + 1, //length of shader source code. 'A' character indicate we want to use ANCI function. +1 is created to accomodate null terminated character at the end of string
		"DS", //name of Macro, which indicates that while compiling create VertexShader
		NULL, // struct of D3D_SHADER_MACRO, if we want to use external shader
		D3D_COMPILE_STANDARD_FILE_INCLUDE, //Include standerd file while compiling the shader program
		"main", //name of entry point function. It is not necessory to use name as 'main'
		"ds_5_0", //shader feature level. Curently level 5 is latest
		0, //Compiler constant. Ex. debug, optimization, validation
		0, // Effect constant. Ex. effect debug, effect optimize, effect validation
		&pID3DBlob_DomainShaderCode, //Compilerd source code in out parameter
		&pID3DBlob_Error  //if any error, will be returned in output parameter
		);
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "Domain shader compilation failed: %s.\n", (char *)pID3DBlob_Error->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Domain shader compilation successful.\n");
		fclose(gpFile);
	}
	...
	*/
	hr = gpID3D11Device->CreateDomainShader(pID3DBlob_DomainShaderCode->GetBufferPointer(),
		...
		NULL,
		&gpID3D11DomainShader); //Out parameter where verted shader object will be stored
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Domain shader object creation failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Domain shader object creation success.\n");
		fclose(gpFile);
	}
...
//*********************************************************************************************************
	...
	bufferDesc_ConstantBuffer.ByteWidth = sizeof(CBUFFER_HAL_SHADER);
	bufferDesc_ConstantBuffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER; //this buffer need to point to cbuffer in shader
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer, nullptr, &gpID3D11Buffer_hal_ConstantBuffer); //create buffer

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Hull Constant buffer creation failed (cbuffer).\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Hull Constant buffer creation success (cbuffer).\n");
		fclose(gpFile);
	}
	...
	bufferDesc_ConstantBuffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER; //this buffer need to point to cbuffer in shader
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer, nullptr, &gpID3D11Buffer_domain_ConstantBuffer); //create buffer

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Domain Constant buffer creation failed (cbuffer).\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Domain Constant buffer creation success (cbuffer).\n");
		fclose(gpFile);
	}
	...
	bufferDesc_ConstantBuffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER; //this buffer need to point to cbuffer in shader
	hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer, nullptr, &gpID3D11Buffer_pixel_ConstantBuffer); //create buffer

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Pixel Constant buffer creation failed (cbuffer).\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Pixel Constant buffer creation success (cbuffer).\n");
		fclose(gpFile);
	}

	gpID3D11DeviceContext->HSSetConstantBuffers(0, 1, &gpID3D11Buffer_hal_ConstantBuffer);
	gpID3D11DeviceContext->DSSetConstantBuffers(0, 1, &gpID3D11Buffer_domain_ConstantBuffer);
	gpID3D11DeviceContext->PSSetConstantBuffers(0, 1, &gpID3D11Buffer_pixel_ConstantBuffer);
...
void display(void)
{
	...
	UINT stride = sizeof(float) * 2; //how many stride for vertex buffer from vertexBuffer data.
	UINT offset = 0; //Offset required 
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer, &stride, &offset);

	gpID3D11DeviceContext->PSSetConstantBuffers(0, 1, &gpID3D11Buffer_pixel_ConstantBuffer); //set constant buffer in device contextS

	gpID3D11DeviceContext->DSSetConstantBuffers(0, 1, &gpID3D11Buffer_domain_ConstantBuffer); //set constant buffer in device contextS

	gpID3D11DeviceContext->HSSetConstantBuffers(0, 1, &gpID3D11Buffer_hal_ConstantBuffer); //set constant buffer in device contextS

	//(start slot ,how many buffers, adress of strides, offset adress)

	// set geometry primitives topology
	gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST); //GL_TRIANGLES. no triangle fan

																						  // define world and view matrix
	XMMATRIX worldMatrix = XMMatrixIdentity();
	XMMATRIX viewMatrix = XMMatrixIdentity();
	viewMatrix = XMMatrixTranslation(0.0f, 0.0f, 2.0f);
	...
	CBUFFER_HAL_SHADER halConstantBuffer;
	ZeroMemory(&halConstantBuffer, sizeof(CBUFFER_HAL_SHADER));
	halConstantBuffer.Hal_Constant_Function_Param = { gNumberOfStripsUniform ,float(gNumberOfLineSegment) };

	CBUFFER_PIXEL_SHADER psConstantBuffer;
	ZeroMemory(&psConstantBuffer, sizeof(CBUFFER_PIXEL_SHADER));
	psConstantBuffer.LineColor = { 1.0f,1.0,1.0,1.0 };


	CBUFFER_DOMAIN_SHADER constantBuffer;
	ZeroMemory(&constantBuffer, sizeof(CBUFFER_DOMAIN_SHADER));
	...
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_domain_ConstantBuffer, 0, NULL, &constantBuffer, 0, 0); //update resource glUniformMatrix
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_hal_ConstantBuffer, 0, NULL, &halConstantBuffer, 0, 0); //update resource glUniformMatrix
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_pixel_ConstantBuffer, 0, NULL, &psConstantBuffer, 0, 0); //update resource glUniformMatrix

																											 //Draw the vertext buffer vertices to renderer view
	gpID3D11DeviceContext->Draw(4, 0); //draw by 3 vertex from vertices and start from 0
