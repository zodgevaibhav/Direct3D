...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_position = NULL; //vertex buffer vbo_position
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_color = NULL; //vertex buffer vbo_color
...
//********************************** Triangle Vertices ******************************************
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_position); //Create VBO
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_position, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource); //Map vertex buffer to this resources.
	...
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_position, 0); //Once copied data to subresources then unmap the vertexBuffer from device context
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_position, &stride, &offset); //for position
