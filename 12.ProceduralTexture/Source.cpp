...
ID3D11PixelShader *gpID3D11PixelShader = NULL; //fragment shader Object
											   ...
											   //ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Position_Pyramid = NULL; //vertex buffer vbo_position
											   //ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Pyramid = NULL;//vertex buffer vbo_texture

ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Position_Cube = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Cube = NULL;

ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Position_Cube_tilted = NULL;
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Texture_Cube_tilted = NULL;
...
ID3D11DepthStencilView *gpID3D11DepthStencilView = NULL; //accesses a texture resource during depth-stencil testing
														 ...
														 //ID3D11ShaderResourceView *gpID3D11ShaderResourceView_Texture_Pyramid = NULL; // specifies the subresources a shader can access during rendering. Examples of shader resources include a constant buffer, a texture buffer, and a texture.
...
char checkImage[checkImageHeight][checkImageWidth][4];
void MakeCheckImage(void);

char checkImage1D[16384];
std::vector<UINT8> GenerateTextureData(void);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szClassName,
		TEXT("D3D: Procedural Texture - Checker bpard"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	*/
	...
	//********************************** cube Vertices tilted ******************************************
	float cube_vertices_tilted[] =
	{
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_vertices_tilted); // size is sizeof(float) * 3 because there are 3 rows in vertices array
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;     //This buffer need to bind to vertex buffer.
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;  // allow CPU to write into this buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Position_Cube_tilted);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed For Vertex Buffer For Cube Position.\n");
	...
	mappedSubresource;
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Cube_tilted, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource); // map buffer
	memcpy(mappedSubresource.pData, cube_vertices_tilted, sizeof(cube_vertices_tilted));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Position_Cube, NULL);
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_texcoords_tilted);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;     //This buffer need to bind to vertex buffer.
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;  // allow CPU to write into this buffer
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Texture_Cube_tilted);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Vertex Buffer creation failed for Cube Texture.\n");
	...
	// copy vertex texcoords into above buffer
	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture_Cube_tilted, NULL, D3D11_MAP_WRITE_DISCARD, NULL, &mappedSubresource); // map buffer
	memcpy(mappedSubresource.pData, cube_texcoords_tilted, sizeof(cube_texcoords_tilted));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture_Cube_tilted, NULL);
	...
	hr = LoadD3DTexture(&gpID3D11ShaderResourceView_Texture_Cube);
	//hr = LoadD3DTexture(L"Smily.bmp", &gpID3D11ShaderResourceView_Texture_Cube);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
...
HRESULT LoadD3DTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	resource.pSysMem = checkImage;
	resource.SysMemPitch = checkImageWidth*4;
	resource.SysMemSlicePitch = checkImageHeight*checkImageWidth;
	...
	hr=gpID3D11Device->CreateTexture2D(&textureDesc, &resource, &pTexture);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateTexture2D() Failed For Texture Resource.\n");
		fprintf_s(gpFile, "CreateTexture2D() ***************** size of data in buffer is \n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateTexture2D() Succeeded For Texture Resource.\n");
		fclose(gpFile);
	}

	D3D11_TEX2D_SRV Texture2Dparams;
	Texture2Dparams.MipLevels = 1;
	Texture2Dparams.MostDetailedMip = 0;
	...
	memset(&shaderResourceDescription, 0, sizeof(shaderResourceDescription));

	shaderResourceDescription.Format = textureDesc.Format;
	shaderResourceDescription.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	shaderResourceDescription.Texture2D = Texture2Dparams;

	_Outptr_opt_ ID3D11ShaderResourceView** textureView=NULL;
	
	hr=gpID3D11Device->CreateShaderResourceView(pTexture, &shaderResourceDescription, ppID3D11ShaderResourceView);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CreateWICTextureFromFile() Failed For Texture Resource.\n");
		fprintf_s(gpFile, "CreateWICTextureFromFile() ***************** size of data in buffer is \n");
		fclose(gpFile);
		return(hr);
	}
	else
...
void display(void)
{
	...
	XMMATRIX translationMatrix = XMMatrixTranslation(-0.8f, 0.0f, 4.0f);

	XMMATRIX scaleMatrix = XMMatrixScaling(0.75f, 0.75f, 0.75f);
	XMMATRIX worldMatrix = scaleMatrix  * translationMatrix; // ORDER IS VERY ... VERY ... VERY ... IMPORTANT
...
//**************************************************************************
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Position_Cube_tilted, &stride, &offset);

	stride = sizeof(float) * 2; // 2 is for u, v
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Texture_Cube_tilted, &stride, &offset); // 1st param '1' is for 1st slot used for 'color'
	...
	viewMatrix = XMMatrixIdentity();
	 ...
	 translationMatrix = XMMatrixTranslation(-0.5f, 0.0f, 3.5f);

	scaleMatrix = XMMatrixScaling(0.75f, 0.75f, 0.75f);
	worldMatrix = scaleMatrix  * translationMatrix; // ORDER IS VERY ... VERY ... VERY ... IMPORTANT
...
{
	// code
	angleTriangle = angleTriangle + 0.002f;
...
void MakeCheckImage(void)
{
	...
	for (i = 0; i<checkImageHeight; i++)
	{
		for (j = 0; j<checkImageWidth; j++)
		{
			...
			checkImage1D[l]= (byte)c;
			checkImage1D[l+1] = (byte)c;
			checkImage1D[l+2] = (byte)c;
			checkImage1D[l+3] = (byte)c;
			l = l + 4;
...
std::vector<UINT8> GenerateTextureData() //One more way to generate Procedural texture
{
	const UINT rowPitch = checkImageWidth *4;
	const UINT cellPitch = rowPitch >> 3;        // The width of a cell in the checkboard texture.
	const UINT cellHeight = checkImageWidth >> 3;    // The height of a cell in the checkerboard texture.
	const UINT textureSize = rowPitch * checkImageHeight;
