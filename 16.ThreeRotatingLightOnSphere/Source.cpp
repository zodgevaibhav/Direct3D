...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szClassName,
		TEXT("D3D11:Single Phong Light on Spher - PerPixel"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	*/
	...
	const char *pixelShaderSourceCode =
		...
		" RvVv = pow(max(dot(reflection_vector, normalized_viewer_vector), 0.0), 50.0f); "\
		" specular = LsKs_mul * RvVv;" \
		"phong_ads_color=float4(ambient + diffuse + specular,1.0)+phong_ads_color;" \
...
void display(void)
{
	...
	if (gbLight == true)
	{
		...
		constantBuffer.Lp_blue_uniform = XMVectorSet(lightPosition_blue[0], lightPosition_blue[1], lightPosition_blue[2], lightPosition_blue[3]);
		constantBuffer.Ld_blue_uniform = XMVectorSet(lightDiffuse_blue[0], lightDiffuse_blue[1], lightDiffuse_blue[2], lightDiffuse_blue[3]);
