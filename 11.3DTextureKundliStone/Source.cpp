...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szClassName,
		TEXT("Two 3D Shapes with Texture"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	*/
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Position_Pyramid); //Create VBO
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Vertext buffer creation failed for position.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position_Pyramid, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource); //Map vertex buffer to this resources.
	...
	memcpy(mappedSubresource.pData, pyramid_vertices, sizeof(pyramid_vertices)); //copy vertertces to mappedSubResource data in pData
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Position_Pyramid, NULL); // //Once copied data to subresources then unmap the vertexBuffer from device context
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Vertext buffer creation failed for Texture.\n");
		fclose(gpFile);
		return(hr);
	}
	else
...
void display(void)
{
	...
	UINT offset = 0;//Offset required 
					...
					//DESCRIBE: IASetVertexBuffers(start slot ,how many buffers, adress of strides, offset adress)
