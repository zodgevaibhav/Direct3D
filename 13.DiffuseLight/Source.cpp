...
HRESULT initialize(void)
{
	...
	*/
	...
	const char *vertexShaderSourceCode =
		...
		"{" \
		"float4x4 world_view_pmatrix;" \
		"float4x4 projection_matrix;" \
		"float4 Ld_uniform;"\
		"float4 Kd_uniform;"\
		"float4 light_position_uniform;"\
		...
		"{"\
		"float4 eyeCoordinated=mul(world_view_pmatrix , pos);"\
		"float3 tnorm=normalize(mul((float3x3)world_view_pmatrix , (float3)normal));"\
		"float3 s=(float3)normalize(light_position_uniform-eyeCoordinated);"\
		"output.diffuse_light= (Ld_uniform * Kd_uniform) * max(dot(s , tnorm) , 0.0);"\
		...
		"}"\
		"float4 localposition = mul(world_view_pmatrix , pos);"
		"output.position =mul(localposition ,projection_matrix);"\
		"return(output);" \
		"}";
	...
	*/
	...
	const char *pixelShaderSourceCode =
		"float4 main(float4 position:POSITION , float4 diffuse_light:COLOR) : SV_TARGET" \
		"{" \
	...
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer, 0); //Once copied data to subresources then unmap the vertexBuffer from device context
																  ...
																  */
																  ...
																  //********************************** Triangle Color ******************************************
	float normal[] = //winding orer is left hand (anti clock wise) winding order for DirectX. OpenGL winding order was right hand
	{
		// front
	...
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC; //Dynamic draw, GL_STATIC_DRAW or GL_DYNAMIC_DRAW
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(normal); //sizeof(float) * sizeof(vertices).
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER; //This buffer need to bind to vertex buffer.
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE; //allowing GPU to write on to this buffer
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_normal, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource); //Map vertex buffer to this resources.
																													  //(vertexBiffer, are we sending multiple data{vertex,color,texture},We want to discrard older data, should we send data now ?(or static or dynamic draw , adress of subrespurce )
	memcpy(mappedSubresource.pData, normal, sizeof(normal)); //copy vertertces to mappedSubResource data in pData
...
void display(void)
{
	...
	if (gbLight == true)
	{
		constantBuffer.Ld_uniform = XMVectorSet(1.0f, 1.0f, 1.0f,1.0f);
		constantBuffer.Kd_uniform = XMVectorSet(0.5f, 0.5f, 0.5f,0.5f);
		constantBuffer.light_position_uniform = XMVectorSet(0.0f, 0.0f, 2.0f, 1.0f);
	...
	constantBuffer.world_view_pmatrix = worldMatrix; //set data in constant buffer, so that we can put it in shader
