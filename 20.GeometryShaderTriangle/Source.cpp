...
ID3D11GeometryShader *gpID3D11GeometryShader = NULL; //fragment shader Object
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szClassName,
		TEXT("D3D11:Geometry Shader 3 Triangle"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
HRESULT initialize(void)
{
	...
	*/
	...
	const char *vertexShaderSourceCode =
		...
		"};"\
		"vertex_out main(float4 pos : POSITION)" \
		"{" \
		"vertex_out output;"
		"output.position=pos;"
		"return(output);" \
	...
	*/
	...
	const char *geometryShaderSourceCode =
		...
		"void main(triangle vertex_out input[3], inout TriangleStream<geometry_out> triangleStream)" \
		"{" \
		"geometry_out output;"\
	...
	//Step 3: Compile the shader
	/*Shader compiler exe name is fxe.exe called as Effect-Compiler tool */
	hr = D3DCompile(geometryShaderSourceCode, // Vertex shader source code
		lstrlenA(geometryShaderSourceCode) + 1, //length of shader source code. 'A' character indicate we want to use ANCI function. +1 is created to accomodate null terminated character at the end of string
		"GS", //name of Macro, which indicates that while compiling create VertexShader
		NULL, // struct of D3D_SHADER_MACRO, if we want to use external shader
		D3D_COMPILE_STANDARD_FILE_INCLUDE, //Include standerd file while compiling the shader program
		"main", //name of entry point function. It is not necessory to use name as 'main'
		"gs_5_0", //shader feature level. Curently level 5 is latest
		0, //Compiler constant. Ex. debug, optimization, validation
		0, // Effect constant. Ex. effect debug, effect optimize, effect validation
		&pID3DBlob_GeometryShaderCode, //Compilerd source code in out parameter
		&pID3DBlob_Error  //if any error, will be returned in output parameter
		);
	...
	if (FAILED(hr))
	{
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "Geometry shader compilation failed: %s.\n", (char *)pID3DBlob_Error->GetBufferPointer());
			fclose(gpFile);
			pID3DBlob_Error->Release();
			pID3DBlob_Error = NULL;
			return(hr);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "Geometry shader compilation successful.\n");
		fclose(gpFile);
	}
	...
	*/
	hr = gpID3D11Device->CreateGeometryShader(pID3DBlob_GeometryShaderCode->GetBufferPointer(),
		...
		NULL,
		&gpID3D11GeometryShader); //Out parameter where verted shader object will be stored
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "geometry shader object creation failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "geometry shader object creation success.\n");
		fclose(gpFile);
	}
	...
	gpID3D11DeviceContext->GSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer); //set constant buffer in device contextS
...
void display(void)
{
	...
	gpID3D11DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP); //GL_TRIANGLES. no triangle fan

																						  // define world and view matrix
	XMMATRIX worldMatrix = XMMatrixIdentity();
	XMMATRIX viewMatrix = XMMatrixIdentity();
	viewMatrix = XMMatrixTranslation(0.0f, 0.0f, 8.0f);
	...
	gpID3D11DeviceContext->Draw(3, 0); //draw by 3 vertex from vertices and start from 0
