...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szClassName,
		TEXT("D3D11:Single Phong Light on Sphere - PerPixel/Fragmet on P/V Key"),
		WS_OVERLAPPEDWINDOW,
		100,
		100,
		WIN_WIDTH,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 0x41: // for 'A' or 'a'
			...
			break;
		case 0x50: // for 'P' or 'p'
			lightPerShaderSwitch = 1;
			break;
		case 0x56: // for 'V' or 'v'
			lightPerShaderSwitch = 2;
			break;
...
HRESULT initialize(void)
{
	...
	*/
	...
	const char *vertexShaderSourceCode =
		...
		"float3 light_direction:LDIRECTION;"\
		"float3 viewer_vector:VVECTOR;"\
		"float4 phong_ads_color:VCOLOR;" \
		"};"\
		...
		"if(L_KeyPressed_uniform==2)"\
		"{"\
		"float3 eyeCoordinated=mul((float3x3)model_matrix , (float3)pos);"\
		"eyeCoordinated=mul((float3x3)view_matrix,eyeCoordinated );"\
		"float3 transformed_normals=normalize(mul(mul((float3x3)view_matrix,(float3x3)model_matrix),(float3)normal));"\
		"float3 light_direction = normalize((float3)Lp_uniform-eyeCoordinated.xyz);"\
		"float tn_dot_ld = max(dot(transformed_normals,light_direction),0.0);"\
		"float3 ambient = mul((float3)La_uniform , (float3)Ka_uniform);" \
		"float3 diffuse = mul(mul(Ld_uniform , Kd_uniform) , tn_dot_ld);" \
		"float3 reflection_vector = reflect(light_direction, transformed_normals);" \
		"float3 viewer_vector = normalize(-eyeCoordinated.xyz);" \
		"float3 LsKs_mul=mul((float3)Ls_uniform , (float3)Ks_uniform);"\
		"float3 RvVv = pow(max(dot(reflection_vector, viewer_vector), 0.0), 50.0f); "\
		"float3 specular = mul(LsKs_mul, RvVv);" \
		"output.phong_ads_color=float4((ambient + diffuse + specular),1.0);"\
		/*saturate can be used inplace of max function in D3D*/
		"}"\
	...
	*/
	...
	const char *pixelShaderSourceCode =
		...
		"if(L_KeyPressed_uniform==2)" \
		"{" \
		"phong_ads_color=input.phong_ads_color;" \
		"}" \
...
void display(void)
{
	...
	XMMATRIX translationaMatrix = XMMatrixTranslation(0.0f, 0.0f, 3.0f);
	...
	if (gbLight == true)
	{
		...
		*/
		...
		constantBuffer.L_KeyPressed_uniform = lightPerShaderSwitch;
